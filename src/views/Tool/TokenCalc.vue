...
const form = reactive({ type: 0, currentToken: '', drawnBox: '', targetBox: '' })
const data = [
    [
...
form.totalToken = computed(() => {
    ...
    for (let i = 0; i < data[form.type].length; i++) {
        ...
        if (form.drawnBox >= value.drawnBox) {
            total =
                parseInt(key) +
                (form.drawnBox - value.drawnBox) * value.perToken;
            break;
        }
    }
...
form.totalBox = computed(() => {
    ...
    for (let i = 0; i < data[form.type].length; i++) {
        ...
        if (form.totalToken >= key) {
            total =
                value.drawnBox +
                parseInt((form.totalToken - parseInt(key)) / value.perToken);
            break;
        }
    }
    return total;
...
form.needToken = computed(() => {
    ...
    for (let i = 0; i < data[form.type].length; i++) {
        ...
        if (form.targetBox >= value.drawnBox) {
            total =
                parseInt(key) +
                (form.targetBox - value.drawnBox) * value.perToken;
            break;
        }
    }
