...
function getArticle(bullet: Bullet[]) {
  ...
  if (targets.length > 0) {
    targets.forEach((target: Article) => {
      ...
      const newItem: Bullet = cloneDeep(source)
...
<template>
  <div>
    <div class="fc flex-wrap mt-2">
      <el-check-tag
        v-for="bulletType, idx in checkBulletGroup" :key="idx"
        class="m-1"
    ...
    <el-scrollbar :max-height="height" class="p-[5px] relative">
      <el-card
        v-for="bulletType, idx in checkBulletGroup.filter((item) => item.checked === true)"
        :key="idx"
        class="m-2 w-auto"
      ...
      >
        <div
          v-for="bullet, idx1 in DATA.filter((item) => item.seq_id.startsWith(bulletType.slotType.toString()))"
          :key="idx1"
      ...
      <div class="absolute bottom-3 right-3">
        <el-dropdown size="small" @command="handleCommand">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item
                v-for="config, idx in DefaultBulletConfig" :key="idx"
                :command="config.list"
              >
                {{ config.name }}
              </el-dropdown-item>
